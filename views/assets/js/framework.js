...
window.onload = () => {
    ...
    fetch("/fetch_user_framework/2020-2024/a")
    .then((res)=>{
    ...
    .then((data)=>{
        ...
        data.forEach(user => {
            ...
            if(user.framework.is_filled==="Not Filled")
            {
                var is_filled=`<div class="filled-box text-danger">${user.framework.is_filled}</div>`
            ...
            {
                var is_filled=`<div class="filled-box text-success">${user.framework.is_filled}</div>`
                var td_status=document.createElement('td')
                td_status.innerHTML=is_filled
                tr.appendChild(td_status)
            }
...
//fetching  the  data  of user  
   ...
   fetch(`/fetch_user_framework/${year}/${section}`)
    .then((res)=>{
